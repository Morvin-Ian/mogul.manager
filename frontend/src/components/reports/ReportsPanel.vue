<template>
  <div class="reports-panel">
    ...
    <template v-else-if="report">
      ...
      <div class="rp-section">
        <h4 class="rp-section-title">Tasks by priority</h4>
        <div class="rp-bars">
          <div v-for="p in report.by_priority" :key="p.priority" class="rp-bar-row">
            <span class="rp-bar-label">{{ priorityLabel(p.priority) }}</span>
            <div class="rp-bar-track">
              <div
                class="rp-bar-fill"
                :class="`rp-bf-p${p.priority}`"
                :style="{ width: pct(p.count) + '%' }"
              ></div>
            </div>
            <span class="rp-bar-count">{{ p.count }}</span>
          </div>
        </div>
      </div>
    </template>
...
const report = computed(() => store.projectReport)
const loading = computed(() => store.loading)

const maxCount = computed(() => {
  if (!report.value) return 1
  const fromStatus = Math.max(...report.value.by_status.map((s) => s.count), 1)
  const fromPriority = Math.max(...report.value.by_priority.map((p) => p.count), 1)
  return Math.max(fromStatus, fromPriority)
})
...
onMounted(() => { store.fetchProjectReport(props.projectId) })
...
.rp-bf-p1           { background: #10B981; }
.rp-bf-p2           { background: #F59E0B; }
.rp-bf-p3           { background: #F97316; }
.rp-bf-p4           { background: #EF4444; }
...
:global([data-theme="dark"]) .rp-card {
  background: #1A2D42;
  border-color: rgba(91, 155, 255, 0.20);
}

:global([data-theme="dark"]) .rp-bar-track {
  background: #253341;
}
